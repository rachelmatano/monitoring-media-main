...
<script>
     function getDataPerMonth() {
        ...
        $.get('{{url("/data_per_month")}}/' + year.value+"-"+month.value+"/"+project.value, function (data) {
            // alert(JSON.stringify(data["online"]));
            ...
            var config = {
                ...
                options: {
                    title:{
                        display:true,
                        text:'Status Contribution on '+month.options[month.selectedIndex].text+' '+year.value +' on '+project.options[project.selectedIndex].text
                    },
                    scales: {
                        yAxes: [{
                            ticks: {
